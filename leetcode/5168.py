...
class Solution(unittest.TestCase):
    def numSmallerByFrequency(self, queries: List[str], words: List[str]) -> List[int]:
        temp_num = [0]*2000
        max_num = 0
        # print("queries",queries,"words",words)
        for x in words:
            n = self.f(x)
            temp_num[n-1] = temp_num[n-1]+1
            if n>max_num:
                max_num=n
        # print(temp_num)
        ...
        result = []
        for x in queries:
            result.append(temp_num[self.f(x)])
        return result

    def f(self, querie: str):
        s = sorted(querie)
        return s.count(s[0])
        #return querie.count(min(querie))


    def setUp(self):
        self.run = self.numSmallerByFrequency
